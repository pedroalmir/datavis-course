...
{
  buildvis;
  let mapInstance = L.map('mapid').setView([-41.05, 172.93], 5)//center [lat, long], zoom
...
L.layerGroup().addTo(map)
  ...
  main.variable(observer("circles")).define("circles", ["circlesLayer","d3","earthquakes","L"], function(circlesLayer,d3,earthquakes,L)
{
  circlesLayer.clearLayers()
  let magScale = d3.scaleLinear().range([5000, 10000]).domain([0, 8])
  earthquakes.forEach(function(d) {
    let circle = L.circle([d.latitude, d.longitude], d.magnitude*11000, {
      color: '#fd8d3c',
      weight: 2,
      fillColor: '#fecc5c',
      fillOpacity: 0.5
