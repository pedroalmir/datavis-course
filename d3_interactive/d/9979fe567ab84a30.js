  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","topojson","us","rateById","showTooltip","hideTooltip"], function(d3,DOM,topojson,us,rateById,showTooltip,hideTooltip)
{
  ...
  svg.append("g")
    ...
    .enter().append("path")
      ...
      .append("title").text(d => "Taxa: " + rateById["$" + d.id] + "%")
...
function showTooltip(county_id, x, y){
  let node = d3.select("#tooltip").style("left", x + "px").style("top", y + "px")
  
  node.select("#countyName").text(county_names.get(county_id))
  node.select("#taxa").text(rateById.get(county_id))
  ...
  main.variable(observer("tooltip")).define("tooltip", ["d3"], function(d3)
{
  ...
  node.append("p").html("<strong id='countyName'></strong><br/>")
  node.append("p").html("Taxa de Desemprego: <span id='taxa'></span>%")
  return node
}
);
